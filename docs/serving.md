...
## Generated UTF-8 recovery

Generated byte-level tokens may split a valid UTF-8 character, including Korean,
across multiple tokens. The decoder buffers a valid incomplete suffix until it
can finish the character. If generated bytes are malformed, each Unicode maximal
subpart is replaced with U+FFFD (`�`) and generation continues. A truncated final
suffix is also replaced. Logical generated token IDs, token usage and GPU/MTP
state are not rewritten. This is output recovery, not a cure for the underlying
source of malformed generated bytes or a guarantee of answer quality.

On request completion, a recovered response produces a console `[warning]`
record with `generated_utf8_repaired`. With `--request-log-jsonl`, the completed
request retains `output_diagnostics.utf8_replacements`, at most eight bounded
`utf8_repair_examples`, and a warning code/action. Examples contain token and
byte diagnostics; normal logging does not capture full input prompts. JSONL
schema version is 9. Repair diagnostics are committed with output, so abandoned
previews do not leak warnings. Response JSON/SSE shapes and finish reasons remain
unchanged; only invalid output text is replaced. CUDA, allocation, or unrelated
internal errors are not silently treated as recoverable UTF-8.
...
| `request_done` | finish reason, prompt/completion/cache/computed-prefill tokens, prefix reuse path, unrounded phase seconds, complete speculative-decoding counters, and bounded generated UTF-8 recovery diagnostics/warnings |
...
The JSONL file contains no full generated response text; recovery examples include bounded
generated-byte hex windows and token IDs. It never records an API-key value; `argv`
